...
describe('browser-native capsules', () => {
  ...
  it('keeps four unique source-bound capsule definitions', () => {
    expect(capsules).toHaveLength(4)
    expect(new Set(capsules.map((capsule) => capsule.commit)).size).toBe(4)
    expect(new Set(capsules.map((capsule) => capsule.expectedReceiptSha256)).size).toBe(4)
  })
})
